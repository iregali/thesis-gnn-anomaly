...
Uses multiprocessing to scan many packages at once.
"""
...
# GuardDog rules grouped into the feature categories from the methodology
RULE_GROUPS = {
    "code_execution": [
...
def load_packages():
    """Load the node list (Name, Version) to scan."""
...
def load_done():
    """Load already-processed packages so we can resume."""
...
def main():
    ...
    print(f"To scan: {len(todo)} using {NUM_WORKERS} workers", flush=True)

    if not todo:
        print("Nothing to scan. Done.", flush=True)
        return
